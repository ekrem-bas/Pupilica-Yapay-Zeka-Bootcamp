...
cizgi1 = go.Scatter(x = df.dunya_siralama, # x ekseninde dunya siralamasi olsun
                    y = df.alinti,
                    mode = "lines+markers", # cizgi ve sacilim grafigi olsun
                    name = "Alinti",
                    marker = dict(color = "rgba(50,243,99,1)"), # renk
                    text = df.uni_isim
                    )

cizgi2 = go.Scatter(x = df.dunya_siralama, # x ekseninde dunya siralamasi olsun
                    y = df.ogretim,
                    mode = "lines+markers", # cizgi ve sacilim grafigi olsun
                    name = "Ogretim",
...
yerlesim = dict(title = "İlk 100 Alıntı Puanı", xaxis = dict(title="dunya siralamasi"))
...
# %%
"""
2015 yilina ait ilk 10 universitenin ogretim skorlarini bar plot olarak, alinti skorlarini ise line+scatter plot olarak cizdirelim
"""

veri2015 = veri[veri.yil == 2015].iloc[:10,:]

bar = go.Bar(x = veri2015.uni_isim,
             y = veri2015.ogretim,
             name = "Ogretim",
             marker = dict(color = "rgba(255, 120, 20, 0.6)", line = dict(color = "rgba(0,100,0,0.5)", width = 1.5)),
             text = veri2015.ulke)

cizgi = go.Scatter(x = veri2015.uni_isim,
               y = veri2015.alinti,
               name = "Alinti",
               marker = dict(color = "rgba(64,0,32,0.5)",
